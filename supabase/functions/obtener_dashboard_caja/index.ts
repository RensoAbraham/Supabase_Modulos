...
serve(async (req) => {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return new Response(JSON.stringify({ error: "Token JWT necesario." }), {
      status: 401,
    });
  }
  ...
  // Validar errores en la consulta del usuario
  if (userError) {
    return new Response(
      JSON.stringify({
  ...
  try {
    ...
    const result = dashboardData && dashboardData.length > 0
      ? dashboardData[0]
      : {
        total_vendido: 0.00,
        total_efectivo: 0.00,
